 ...
 *
 */
public class FEAT_alpha_random extends ICNPBasic {
...
//	
	...
	public FEAT_alpha_random(){
		ptname = new String("FEAT_alpha_random");
	}
	...
	double execute(){
		...
		while(targetset.size()>0){ //当服务器目标标签被全部识别，协议就可以终止
			...
			double opt_alpha;// = sample_and_interpolation(sample_num, interplotion_num)[0];//取第0个值得到最优opt_alpha
			opt_alpha = Math.random(); //生成0-1之间的随机数
