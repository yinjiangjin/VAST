 ...
 *
 */
public class FEAT_min extends ICNPBasic {
...
//	
	...
	public FEAT_min(){
		ptname = new String("FEAT_min");
	}
	
	double execute(){
		
		System.out.println("********start FEAT_min  new protocol********** ");
		...
		while(targetset.size()>0){ //当服务器目标标签被全部识别，协议就可以终止
			...
			Utils.logAppend("FEAT_min _new.txt", "optf: "+opt_frame+"  ");		
			OurKEYMissing_testingAndhopping(opt_frame); //根据最优帧长执行协议
			
		}
		Utils.logAppend("FEAT_min _new.txt", "optf: "+"\n");
		...
		System.out.println("****End FEAT_min  new protocol***** tcollection******:"+ tcollection);
		return tcollection;
	}
