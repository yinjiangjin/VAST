...
public class GraphReader {
    ...
    public static Graph readGraph() throws Exception
    {
        ...
        StringTokenizer token = new StringTokenizer(line, " "); //空格作为分隔符
        token.nextToken(); //e
        token.nextToken(); //1
        Constants.NUMBER_NODES = Integer.parseInt(token.nextToken().trim()); //16
        Graph graph = new Graph();
        line = reader.readLine();
        while(line != null)
        {
            token = new StringTokenizer(line, " ");
            token.nextToken(); //跳过字母e
            int sv = Integer.parseInt(token.nextToken().trim()); //start vertex
